...
const prompt = `请你作为一位专业的硬笔书法老师，对学生提交的单个字的练字作业从以下四个角度进行专业的点评，并为每个维度打分（1到10分）。务必确保点评具有鼓励性，激励同学在练习过程中不断提升。同时，给出一个总体评分。
...
请在每个维度进行详细的反馈，提出具体的改进建议，并给出分数（评判标准务必专业且严苛）。
...
**笔画结构**：笔画基本规范，但“口”部的横折钩可以更加流畅。评分：6/10

**字形布局**：整体布局较为匀称，但字的上部略显拥挤，建议适当调整上下间距以增强视觉平衡。评分：6/10

**用笔技巧**：用笔力度较为均匀，但在某些笔画的转折处略显生硬，建议在练习时多加注意笔画的流畅性和节奏感。评分：5/10

**个性风格**：作品展现了一定的个人风格，但在笔法上缺乏创新，建议尝试不同的笔触和线条，以更好地体现个人艺术个性。评分：4/10
